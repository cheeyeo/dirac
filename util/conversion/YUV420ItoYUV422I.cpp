/* ***** BEGIN LICENSE BLOCK *****
*
* $Id: YUV420ItoYUV422I.cpp,v 1.1 2008/11/18 23:25:21 asuraparaju Exp $ $Name: Dirac_1_0_2 $
*
...
stored in raw YUV422 format, by vertically interpolating taking into
account the chroma sample positions in each field.
...
int main(int argc, char * argv[] ) {
    ...
    if (argc != 4) {
        cout << "\"YUV420ItoYUV422I\" command line format is:" << endl;
        ...
        cout << "    Example: YUV420ItoYUV422I <foo >bar 720 576 3" << endl;
    ...
    for (int frame=0; frame<frames; ++frame) {
        ...
        // top two lines of chroma output
        for (int x=0; x<width2; ++x ){
            UBufferOut[x] = UBufferIn[x];
            UBufferOut[width2+x] = UBufferIn[width2+x];
            VBufferOut[x] = VBufferIn[x];
            VBufferOut[width2+x] = VBufferIn[width2+x];
        }

        // middle lines
        for (int line_out=2,line_in=0; line_out<height-2; line_out+=4,line_in+=2) {
            for (int x=0; x<width2; ++x ){
                UBufferOut[(line_out+0)*width2+x] = (5*UBufferIn[(line_in+0)*width2+x]
                                                  + 3*UBufferIn[(line_in+2)*width2+x]
                                                  ) >> 3;
                UBufferOut[(line_out+2)*width2+x] = (1*UBufferIn[(line_in+0)*width2+x]
                                                  + 7*UBufferIn[(line_in+2)*width2+x]
                                                  ) >> 3;
                UBufferOut[(line_out+1)*width2+x] = (7*UBufferIn[(line_in+1)*width2+x]
                                                  + 1*UBufferIn[(line_in+3)*width2+x]
                                                  ) >> 3;
                UBufferOut[(line_out+3)*width2+x] = (3*UBufferIn[(line_in+1)*width2+x]
                                                  + 5*UBufferIn[(line_in+3)*width2+x]
                                                  ) >> 3;

                VBufferOut[(line_out+0)*width2+x] = (5*VBufferIn[(line_in+0)*width2+x]
                                                  + 3*VBufferIn[(line_in+2)*width2+x]
                                                  ) >> 3;
                VBufferOut[(line_out+2)*width2+x] = (1*VBufferIn[(line_in+0)*width2+x]
                                                  + 7*VBufferIn[(line_in+2)*width2+x]
                                                  ) >> 3;
                VBufferOut[(line_out+1)*width2+x] = (7*VBufferIn[(line_in+1)*width2+x]
                                                  + 1*VBufferIn[(line_in+3)*width2+x]
                                                  ) >> 3;
                VBufferOut[(line_out+3)*width2+x] = (3*VBufferIn[(line_in+1)*width2+x]
                                                  + 5*VBufferIn[(line_in+3)*width2+x]
                                                  ) >> 3;
            }
        }
        ...
        // bottom lines
        for (int x=0; x<width2; ++x ){
            UBufferOut[(height-2)*width2+x] = UBufferIn[(height2-2)*width2+x];
            UBufferOut[(height-1)*width2+x] = UBufferIn[(height2-1)*width2+x];
            VBufferOut[(height-2)*width2+x] = VBufferIn[(height2-2)*width2+x];
            VBufferOut[(height-1)*width2+x] = VBufferIn[(height2-1)*width2+x];
        }
